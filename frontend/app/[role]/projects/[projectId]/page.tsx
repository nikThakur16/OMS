...
import { HiOutlineSearch, HiOutlineFilter, HiOutlineSortAscending, HiPlus, HiViewBoards, HiViewList, HiOutlinePencil, HiOutlineTrash, HiOutlineEye } from 'react-icons/hi';
...
function KanbanTaskCard({ task, onView, onEdit, onDelete, listeners, attributes, isDragging, style }: {
  task: TaskType;
  onView: () => void;
  onEdit: () => void;
  onDelete: () => void;
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      className={`mb-3 p-4 rounded-xl shadow bg-gradient-to-br from-white/80 to-${task.priority === 'high' ? 'red' : task.priority === 'medium' ? 'yellow' : 'blue'}-100 border-l-4 ${task.priority === 'high' ? 'border-red-400' : task.priority === 'medium' ? 'border-yellow-400' : 'border-blue-400'} cursor-pointer transition ${isDragging ? 'opacity-60' : ''}`}
      style={style}
    ...
    >
      <div className="flex justify-between items-start">
        ...
        <div className="flex gap-1">
          <button onClick={e => { e.stopPropagation(); onView(); }} className="text-gray-400 hover:text-blue-600 transition"><HiOutlineEye /></button>
          <button onClick={e => { e.stopPropagation(); onEdit(); }} className="text-gray-400 hover:text-green-600 transition"><HiOutlinePencil /></button>
          <button onClick={e => { e.stopPropagation(); onDelete(); }} className="text-gray-400 hover:text-red-600 transition"><HiOutlineTrash /></button>
        </div>
      </div>
...
function SortableTask({ task, onView, onEdit, onDelete }: {
  task: TaskType;
  onView: () => void;
  onEdit: () => void;
  onDelete: () => void;
  ...
  return (
    <div ref={setNodeRef} style={style}>
      <KanbanTaskCard task={task} onView={onView} onEdit={onEdit} onDelete={onDelete} listeners={listeners} attributes={attributes} isDragging={isDragging} />
    </div>
  );
}
...
export default function ProjectDetailsPage() {
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-xl text-gray-500">Loading...</div>;
  }
  if (error || !project) {
    return <div className="p-8 text-center text-xl text-gray-500">Project not found.</div>;
  }

  // Kanban drag logic (dummy, no real drag for now)
  function handleCardClick(task: TaskType, mode: 'view' | 'edit' | 'create' = 'view') {
    ...
    <div className="relative rounded-3xl p-6 bg-gradient-to-r from-indigo-500 via-blue-500 to-fuchsia-500 shadow-xl mb-8 overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          {project.description && (
            <div className="text-white/80 text-sm mb-2">{project.description}</div>
          )}
          {project.manager && (
            <div className="text-white/80 text-sm">Manager: <span className="font-bold text-white">{project.manager}</span></div>
          )}
          {project.teamIds && project.teamIds.length > 0 && (
            <div className="text-white/80 text-sm">Teams: {project.teamIds.join(', ')}</div>
          )}
          {project.departmentIds && project.departmentIds.length > 0 && (
            <div className="text-white/80 text-sm">Departments: {project.departmentIds.join(', ')}</div>
          )}
        </div>
      </div>
    </div>
  ...
  const KanbanBoard = () => (
    <DndContext
      ...
      onDragEnd={({ active, over }) => {
        ...
        const fromCol = statusColumns.find(col => tasks.some(t => t.id === active.id && t.status === col.id));
        const toCol = statusColumns.find(col => col.id === over.id || tasks.some(t => t.id === over.id && t.status === col.id));
        if (!fromCol || !toCol) return;
        if (fromCol.id === toCol.id) return;
        setTasks(tasks => tasks.map(t => t.id === active.id ? { ...t, status: toCol.id } : t));
      }}
    >
  ...
  const TaskList = () => (
    <div className="rounded-2xl bg-white/60 border border-white/30 backdrop-blur-md shadow-xl p-4">
      <table className="w-full text-left">
        ...
        <tbody>
          {tasks.map(task => (
            <tr key={task.id} className="border-b border-gray-200 hover:bg-white/80 transition">
              ...
              <td>
                ...
                <button onClick={() => setTasks(tasks.filter(t => t.id !== task.id))} className="text-gray-400 hover:text-red-600 mx-1"><HiOutlineTrash /></button>
              </td>
            </tr>
          ))}
        </tbody>
            ...
            {modalMode === 'view' && selectedTask && (
              <div>
                ...
                <div className="flex gap-2 mb-2">
                  {selectedTask.tags?.map((tag: string) => <span key={tag} className="bg-blue-100 text-xs px-2 py-1 rounded-full">{tag}</span>)}
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700" onClick={() => { setTasks(tasks.filter(t => t.id !== selectedTask.id)); setShowModal(false); }}>Delete</button>
                </div>
              </div>
            )}
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-[#e0e7ff] to-[#f4fafd] min-h-screen relative">
      ...
      {tasks && tasks.length > 0 ? (
        ...
        <div className="text-center text-gray-500 text-lg py-12">No tasks for this project.</div>
      )}
    </div>
  );
} 
...
const page = () => {
  ...
  const filteredUserList = Array.isArray(users)
    ? users.filter((user) => {
        ...
        const department =
          user.personalDetails?.department?.toLowerCase() || "";
    ...
    <div className="w-full h-[82vh] overflow-auto bg-white text-[#034F75] rounded-[12px]">
      <div className="px-4 py-6">
        <div className="flex items-center justify-between p-4 mb-6">
          <h3 className="font-bold tracking-wider text-[24px]">
            Employee Management
          </h3>
        ...
        <table className="w-full">
          ...
          <tbody className=" ">
            {filteredUserList.map((user, index) => (
              <tr
                key={index}
                className="border-b border-zinc-100 text-sm font-medium tracking-wider"
              >
                ...
                <td className="text-center py-4">
                  {user.contactDetails.primaryPhoneNumber}
                </td>
                <td className="text-center py-4">
